...
def load_hybrid_config(path):
    print(f"[DEBUG] Loading hybrid config: {path}")
    try:
        ...
        print(f"[DEBUG] Failed to load hybrid config: {str(e)}")
...
def infer_feature_names(mtype, mobj):
    ...
    if mtype=="lgbm":
        try: return mobj.feature_name() if hasattr(mobj, "feature_name") else None
        except: return None
    if mtype=="xgb":
...
def run_predict(mtype, mobj, X):
    print(f"[DEBUG] Running prediction for model type: {mtype}")
    try:
        if mtype=="sklearn":
        ...
        if mtype=="lgbm":
            if hasattr(mobj, "predict_proba"):
                p = mobj.predict_proba(X)[:,1]
            else:
                p = mobj.predict(X)
            return float(p[0] if np.ndim(p)==1 else p[0,1])
        if mtype=="xgb":
        ...
        print(f"[DEBUG] Prediction failed for {mtype}: {str(e)}")
        raise RuntimeError(f"Prediction failed for {mtype}: {str(e)}")

def try_load_model(path, member_type=None, arch=None):
    print(f"[DEBUG] Attempting to load model: {path}, type_hint={member_type}")
    ...
    if ext in (".pkl",".joblib") and mt not in ("xgb","tabnet","lstm","tcn"):
    ...
    if ext==".json" or mt=="xgb":
        ...
        print(f"[DEBUG] Loaded xgboost model from {path}")
        return "xgb", booster

    if mt=="lgbm" or ext in (".txt",".lgb",".pkl"):
        lgb=_lgb()
        if not lgb: raise RuntimeError("lightgbm not installed")
        if ext in (".txt",".lgb"):
            booster=lgb.Booster(model_file=path)
            print(f"[DEBUG] Loaded lightgbm Booster from {path}")
            return "lgbm", booster
        else:  # .pkl
            try:
                model = jb.load(path)
                ...
                print(f"[DEBUG] Loaded lightgbm Classifier from {path}")
                return "lgbm", model
            except Exception as e:
                print(f"[DEBUG] Failed to load lightgbm as Classifier, trying Booster: {str(e)}")
                try:
                    booster = lgb.Booster(model_file=path)
                    print(f"[DEBUG] Loaded lightgbm Booster from {path}")
                    return "lgbm", booster
    ...
    if ext==".zip" or mt=="tabnet":
        ...
        m = TabNetClassifier()
        m.load_model(path)
        print(f"[DEBUG] Loaded tabnet model from {path}")
        return "tabnet", m
    ...
    if ext==".pth" or mt in ("lstm","tcn"):
        ...
        print(f"[DEBUG] Loaded {mt or 'lstm'} model from {path}")
    ...
    if jb is not None:
        try:
            obj=jb.load(path)
            print(f"[DEBUG] Loaded sklearn model (fallback) from {path}")
            return "sklearn", obj
        except Exception:
            pass
...
def plot_feature_distribution(X, feature_names):
    ...
    X.boxplot(column=feature_names, ax=ax, vert=False)
...
def plot_prediction_probabilities(df_res):
    fig, ax = plt.subplots(figsize=(8, 5))
    valid_probs = df_res[df_res["prob_cal"].notna()]
    ax.bar(valid_probs["disease"], valid_probs["prob_cal"], color='#1E88E5')
    ax.set_title("Prediction Probabilities by Disease")
    ax.set_xlabel("Disease")
    ax.set_ylabel("Calibrated Probability")
    ax.set_ylim(0, 1)
    for i, v in enumerate(valid_probs["prob_cal"]):
...
def predict_member(path, mtype_hint, X, arch=None):
    print(f"[DEBUG] Predicting with member: {path}, type_hint={mtype_hint}")
    try:
        ...
        print(f"[DEBUG] Prediction successful: prob={p}, mtype={mtype}")
        return float(p), "ok", "", mtype, mobj
    except Exception as e:
        print(f"[DEBUG] Member prediction failed: {str(e)}")
        return np.nan, "error", str(e), None, None

def predict_hybrid(hjson_path, X):
    print(f"[DEBUG] Processing hybrid model: {hjson_path}")
    try:
        ...
        probs_all, weights_all, logs, model_objects = [], [], [], []
        ok_count = 0
        pos_count = 0
        for m in members:
            ...
            print(f"[DEBUG] Loading hybrid member: {pth}, type={mtype}, weight={w}")
            p, st, err, mt, mo = predict_member(pth, mtype, X, arch=arch)
            ...
            if np.isfinite(p):
                ...
                if p >= member_thr:
                    pos_count += 1
            if mt and mo:
                model_objects.append({"mtype": mt, "mobj": mo, "path": pth, "weight": w})
            print(f"[DEBUG] Member result: prob={p}, status={st}, error={err}")
        ...
        if ok_count == 0:
            ...
            error_msg = f"All hybrid members failed: {'; '.join([log['error'] for log in logs if log['error']])}"
        elif gated:
            status = f"gated_off({pos_count}/{min_members})"
            error_msg = f"Insufficient positive members ({pos_count}/{min_members})"
        elif ok_count < len(members):
            status = "partial_ok"
            error_msg = f"Some members failed: {'; '.join([log['error'] for log in logs if log['error']])}"
        else:
            status = "ok"
            error_msg = ""

        print(f"[DEBUG] Hybrid prediction: prob={p_final}, status={status}, ok_count={ok_count}/{len(members)}")
        return float(p_final), logs, status, model_objects
    except Exception as e:
        print(f"[DEBUG] Hybrid prediction failed: {str(e)}")
...
def run_predictions(mt_path, known_glob, multi_mode, df_days_minutes):
    try:
        ...
        if not files: 
            st.warning("No known-case CSVs matched; using master table F1 scores.")
        ...
        # Select best OS based on known_df F1 score
        best_os={}
        ...
        for disease in ["mastitis","lameness","calving","oestrus"]:
            ...
            print(f"[DEBUG] Loading model for {disease}: model_name={model_name}, model_path={model_path}")
            if not model_path or model_name == "none":
                print(f"[DEBUG] No valid model for {disease}")
                ...
                model_objects[disease] = (None, None, None)
                continue

            if is_hybrid_config(model_path):
                print(f"[DEBUG] Processing hybrid model for {disease}: {model_path}")
                try:
                    p_raw, member_logs, load_status, hybrid_models = predict_hybrid(model_path, X)
                    error_msg = "" if load_status in ("ok","partial_ok") else f"No valid member probabilities: {'; '.join([log['error'] for log in member_logs if log['error']])}"
                    ...
                    model_objects[disease] = ("hybrid", hybrid_models, model_path)
                    print(f"[DEBUG] Hybrid model status: {load_status}, members: {member_logs}")
                except Exception as e:
                    p_raw=np.nan; member_logs=[]; load_status="hybrid_error"; error_msg=str(e)
                    debug_rows.append({"disease":disease,"chosen_os":os_level,"model":"hybrid","model_path":model_path,"load_status":load_status,"error":error_msg,"members":json.dumps(member_logs)})
                    model_objects[disease] = (None, None, None)
                    print(f"[DEBUG] Hybrid model failed: {error_msg}")
            else:
                try:
                    ...
                    model_objects[disease] = (mtype, mobj, model_path) if mtype and mobj else (None, None, None)
                    print(f"[DEBUG] Single model loaded: mtype={mtype}, status={load_status}")
                except Exception as e:
                    p_raw=np.nan; load_status="error"; error_msg=str(e); mtype, mobj = None, None
                    debug_rows.append({"disease":disease,"chosen_os":os_level,"model":model_name,"model_path":model_path,"load_status":load_status,"error":error_msg})
                    model_objects[disease] = (None, None, None)
                    print(f"[DEBUG] Single model failed: {error_msg}")
        ...
        return chosen_df, df_res, df_res_styled, model_debug_df, summary_df, best_os, comments, winner, X, model_objects, mt_path
    except Exception as e:
...
if st.button("Run Prediction 🧠") and master_table:
    with st.spinner("Analyzing cow health data..."):
        ...
        try:
            chosen_df, df_res, df_res_styled, model_debug_df, summary_df, best_os, comments, winner, X, model_objects, mt_path = run_predictions(
            ...
            st.markdown("<div class='section-header'>Input Feature Distribution</div>", unsafe_allow_html=True)
            fig_features = plot_feature_distribution(X, EIGHT_FEATURES)
            st.pyplot(fig_features)

            st.markdown("<div class='section-header'>Prediction Probabilities</div>", unsafe_allow_html=True)
            fig_probs = plot_prediction_probabilities(df_res)
            st.pyplot(fig_probs)
